...
export type ConnectFourMinimax = Readonly<{
    ...
    min(
        ...
        for (const move of currentPlayer.legalMoves) {
            ...
            currentLowest = Math.min(
                currentLowest,
                this.max(latestBoard, depth - 1, highestValue, currentLowest)
            );
    ...
    max(
        ...
        for (const move of currentPlayer.legalMoves) {
            ...
            currentHighest = Math.max(
                currentHighest,
                this.min(latestBoard, depth - 1, currentHighest, lowestValue)
            );
