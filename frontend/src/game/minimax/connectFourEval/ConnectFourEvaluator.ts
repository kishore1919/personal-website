...
export const evaluateConnectFourBoard = (board: BoardType, league: League, depth: number, searchDepth: number): number => {
    if (depth === 0) {
        return numberOfConsecutivePieceEval(league, board, board.secondPlayer.league, board.firstPlayer.league);
    }
    return numberOfConsecutivePieceEval(league, board, board.secondPlayer.league, board.firstPlayer.league) * (searchDepth - depth);
};

const numberOfConsecutivePieceEval = (league: League, board: BoardType, currentLeague: League, enemyLeague: League): number => {

    let score = 0;

    const vertical = getAllVerticalColumn(board);
    for (let k = 0; k < vertical.length; k++) {
        const eLeagues = vertical[k], maxRun = 3;
        let currentTile = 0, emptyTile = 0, enemyTile = 0;
        let begin = 0, max = 3, i = 0;
    ...
    for (let k = 0; k < horizontal.length; k++) {
        const eLeagues = horizontal[k], maxRun = 4;
        let begin = 0, max = 3, currentTile = 0, emptyTile = 0, enemyTile = 0, i = 0;
        for (let j = begin; j <= max; j++) {
            if (eLeagues[j] === currentLeague) {
                currentTile++;
            }
...
const computeScore = (league: League, currentTile: number, enemyTile: number, emptyTile: number): number => {
...
const getAllHorizontalRow = (board: BoardType): ReadonlyArray<ReadonlyArray<League | null>> => {
    ...
    for (let i = begin; i < connectFour.numberOfTiles; i++) {
        ...
        leagues.push(tile.isTileOccupied && tile.getPiece !== null ? tile.getPiece.league : null);
...
const getAllVerticalColumn = (board: BoardType): ReadonlyArray<ReadonlyArray<League | null>> => {
    const listOfLeagues: Array<ReadonlyArray<League | null>> = [];
    let begin = 0, max = connectFour.column * 5;
    let leagues: Array<League | null> = [];
...
const getPositiveSlopeRow = (board: BoardType): ReadonlyArray<ReadonlyArray<League | null>> => {
    const listOfLeagues: Array<ReadonlyArray<League | null>> = [];
    const increment = connectFour.column - 1;
    let begin = (connectFour.column - 1) / 2, max = 21, goEdge = false, leagues: Array<League | null> = [];
    for (let i = begin; i <= max; i += increment) {
        ...
        if (i === max) {
            ...
            max = max + connectFour.column >= connectFour.numberOfTiles ? max + 1 : max + connectFour.column;
            i = begin - increment;
        }
    }
    return listOfLeagues;
...
const getNegativeSlopeRow = (board: BoardType): ReadonlyArray<ReadonlyArray<League | null>> => {
    const listOfLeagues: Array<ReadonlyArray<League | null>> = [];
    const increment = connectFour.column + 1;
    let begin = (connectFour.column - 1) / 2, max = 27, goEdge = false, leagues: Array<League | null> = [];
    for (let i = begin; i <= max; i += increment) {
        const tile = board.tileList[i];
        leagues.push(tile.isTileOccupied && tile.getPiece !== null ? tile.getPiece.league : null);
        if (i === max) {
