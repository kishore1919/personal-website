...
export const createTicTacToeMove = (league: League, index: number): MoveType => {
    return {
        piece: createPiece(league, index),
        equals(move: MoveType): boolean {
            return move.piece.index === this.piece.index && move.piece.league === this.piece.league
        },
...
export const createConnectFourMove = (league: League, index: number): MoveType => {
    return {
        piece: createPiece(league, index),
        equals(move: MoveType): boolean {
            return move.piece.index === this.piece.index && move.piece.league === this.piece.league
